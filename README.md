...
| `optimistic_default` | Marks a write-only command register: never read it — the entity shows this seed value until written, then optimistically its last written value (an option label for a `select`, a number for a `number`). For "direct control" registers that echo nothing useful, or share an address with an unrelated read. Cannot be combined with `read_register` |
