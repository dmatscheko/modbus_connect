...
async def test_update_exception() -> None:
    ...
    with patch("custom_components.modbus_connect.select._LOGGER.warning") as warning, patch(
        "custom_components.modbus_connect.select._LOGGER.debug"
    ) as debug, patch("custom_components.modbus_connect.select._LOGGER.error") as error:
        entity._handle_coordinator_update()  # pylint: disable=protected-access
...
async def test_update_value() -> None:
    ...
    type(entity).entity_description = PropertyMock(
        return_value=ModbusSelectEntityDescription(  # pylint: disable=unexpected-keyword-arg
            key="key",
            register_address=1,
            control_type="select",
            select_options={1: "A", 2: "B"},
