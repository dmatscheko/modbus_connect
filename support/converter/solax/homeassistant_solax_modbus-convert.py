...
``write_owned``), so ``write_augmented`` **skips** them: it skips any device whose
device.yaml exists, so a re-import can never clobber the hand curation there. They
remain as templates. To bring up a NEW SolaX model (X1, another X3, an EV charger, …)
copy a target, point it at the not-yet-owned device, and it writes
``device_configs/<slug>.yaml`` normally (add its basename→slug mapping to
``support/converter/_common/device_folders.json`` first, unless you name the target
after an existing devicedocs folder).
...
def run(module, spec_fn, meta, filename, extras=(), templates=(), basic=frozenset(), protocol=None,
    ...
    if augment.write_augmented(
        ...
        return  # owned in-tree (device.yaml present) — write_augmented reported the skip
