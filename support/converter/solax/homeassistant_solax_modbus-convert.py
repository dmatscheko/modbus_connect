...
seeded ``static_value`` controls.

The two worked targets below — X3-Hybrid G4 and X3-HAC — are now **owned in-tree**
(``support/devicedocs/<slug>/device.yaml``, regenerated by ``convert_all`` via
``write_owned``), so ``run`` **skips** them: a re-import must never clobber the hand
curation there. They remain as templates. To bring up a NEW SolaX model (X1, another
X3, an EV charger, …) copy a target, point it at the not-yet-owned device, and it
writes ``device_configs/<slug>.yaml`` normally (add its basename→slug mapping to
``support/converter/_common/device_folders.json`` first).
...
def run(module, spec_fn, meta, filename, extras=(), templates=(), basic=frozenset(), protocol=None,
    ...
    slug = augment.folder_for(name)
    if slug in augment.OWNED_DEVICES:
        # Owned in-tree: support/devicedocs/<slug>/device.yaml is the source of truth
        # (regenerated by convert_all's write_owned). A re-import must never clobber the
        # hand curation there, so we skip. The two targets below stay as worked examples
        # for bringing up a NEW SolaX model — copy one, point it at a not-yet-owned
        # device, and it writes device_configs/<slug>.yaml normally.
        print(f"SKIP {filename}: owned in-tree — see support/devicedocs/{slug}/device.yaml")
        return
