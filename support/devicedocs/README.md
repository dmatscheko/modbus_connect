...
Primary-source Modbus documentation for every bundled device, plus a
generated `registers.md` per device that tabulates the registers the
Modbus Connect device file actually reads and writes (register · table ·
Modbus function code · data type / conversion).

Each `registers.md` table is generated from the device YAML in
`custom_components/modbus_connect/device_configs/`; the linked primary source
is the manufacturer's authoritative, complete register map.

| Device | Registers | Primary source | Authority | Addresses |
| --- | --- | --- | --- | --- |
| Dimplex Sole/Wasser-Wärmepumpe SI 11TU | [`dimplex-si-11tu`](./dimplex-si-11tu/registers.md) (56) | [Dimplex — NWPM Modbus TCP Datenpunktliste (Wärmepumpenmanager)](https://dimplex.atlassian.net/wiki/spaces/DW/pages/2873393288/NWPM+Modbus+TCP) | official | verified |
...
| Pichler Lüftungsgerät LG 150 - LG 250 | [`pichler-lg150-lg250`](./pichler-lg150-lg250/registers.md) (43) | [Pichler / LS-Control — Modbus register list (controller ES1015, LG150AB/LG250A)](https://www.pichlerluft.at/unterlagen.html?file=files/content/downloads/LOGIN/LG%20150AB-LG250A/LIST_Modbus_ES1015_FW_LG150AB_LG250A_v2.0.0.xlsx) | official | verified |
| Pichler Lüftungsgerät LG 350 - LG 450 | [`pichler-lg350-lg450`](./pichler-lg350-lg450/registers.md) (131) | [Pichler / LS-Control — Modbus register list (controller ES2020, LG350/450/740/1000)](https://www.pichlerluft.at/unterlagen.html?file=files/content/downloads/LOGIN/LG%20350_LG%20450_LG%20740_LG%201000%20SK/LIST_Modbus_ES2020_FW_LG350_LG450_LG740_LG1000_v2.0.0.xlsx) | official | verified |
...
**Authority** — *official*: fetched from the manufacturer's own site/CDN/wiki. 
*manufacturer doc, community-hosted*: a genuine manufacturer-authored document 
that the vendor does not publish publicly (SolaX, Growatt), mirrored by a 
reputable community project. *none*: no public manufacturer register document exists.

**Addresses** — whether the register addresses in the device file were 
cross-checked against the primary source: *verified* (match), *partial* 
(model/scaling match but some addresses differ or are firmware-assigned).

> THE PRIMARY-SOURCE MODBUS DOCUMENTATIONS FOR THE BUNDLED DEVICES ARE NOT COVERED BY THE LICENSE OF THIS PROJECT!